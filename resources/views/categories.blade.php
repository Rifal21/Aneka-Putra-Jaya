...
<div class="flex overflow-x-auto ml-3 mb-10 flex-wrap justify-center">
        @foreach ($categories as $category)
          ...
          <div class="max-w-sm rounded overflow-hidden shadow-lg m-4 px-5 mb-5">
            @if ($category->sample)
              <img src="{{ asset('storage/' . $category->sample) }}" class="w-full" alt="">
            @else
              <img src="../../img/produk.jpg" class="w-full" alt="">
            @endif
            <div class="px-6 py-4">
              <div class="font-bold text-xl mb-2 text-center uppercase hover:text-secondary"><a href="/produk?category={{ $category->slug }}">{{ $category->name }}</a></div>
            </div>
